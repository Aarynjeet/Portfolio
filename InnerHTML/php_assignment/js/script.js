/*
 * script.js — Memory Match Game (Server-side version)
 * CS 1XD3 Server-side Assignment
 * Author: Aarynjeet Gill
 * Date:   2026-03
 *
 * Adapted from the JS Individual Assignment.
 * Changes from original:
 *   - HistoryManager (localStorage) removed — DB handles history now
 *   - GameController.endGame() populates hidden form inputs and
 *     auto-submits to leaderboard.php after a short delay
 *   - Score/moves are tracked and sent via GET to leaderboard.php
 *   - Quit button (rendered by PHP) gets live score/moves updated
 *     so that even an early quit sends meaningful data
 *
 * Architecture:
 *   Model      — Card, GameState
...
window.addEventListener("load", function () {
    ...
    // Only run if we're on play.php with a valid email
    if (typeof window.PLAYER_EMAIL === "undefined" || !window.PLAYER_EMAIL) {
        return;
    }
    ...
    class SoundManager {
        ...
        playFlip()          { this.playTone(800, "sine",     0.08, 0.2,  0); }
        playMatch()         {
        ...
        playClick()         { this.playTone(600, "sine",      0.06, 0.15, 0); }
        playPeekEnd()       {
    ...
    class GameState {
        ...
        getCurrentConfig() { return this.roundConfigs[this.currentRound]; }
        getTotalPairs()    { return this.getCurrentConfig().pairs; }
        isRoundComplete()  { return this.matchedPairs >= this.getTotalPairs(); }
        isGameComplete()   { return this.currentRound >= this.totalRounds; }
    ...
    class GameView {
        ...
        showHelp() { this.showOverlay(); this.helpPanel.classList.remove("hidden"); }
        hideHelp() { this.helpPanel.classList.add("hidden"); this.hideOverlay(); }

        showRoundPopup(text, isFinal) {
            this.roundPopupText.textContent = text;
            this.nextRoundBtn.textContent   = isFinal ? "See Results" : "Next Round \u2192";
            this.showOverlay();
         ...
         * Populates hidden form inputs so leaderboard.php gets the data.
         * Does NOT include localStorage history (removed for server-side).
         *
         * @param {number} totalScore
         * @param {number} totalMoves
         */
        showEndScreen(totalScore, totalMoves) {
            this.finalScoreText.textContent = "Final Score: " + totalScore;
            this.finalMovesText.textContent = "Total Moves: " + totalMoves;

            // Populate the hidden form that posts to leaderboard.php
            var scoreInput = document.getElementById("end-score-input");
            var movesInput = document.getElementById("end-moves-input");
            if (scoreInput) scoreInput.value = totalScore;
            if (movesInput) movesInput.value = totalMoves;
    ...
    class GameController {
        ...
        init() {
            ...
            setTimeout(function () { self.view.showStartButton(); }, 2000);
            this.attachEvents();
        }
        ...
        attachEvents() {
            ...
            // Quit button — update score/moves before the form submits
            var quitBtn = document.getElementById("quit-btn");
            if (quitBtn) {
                quitBtn.addEventListener("click", function () {
                    var scoreInput = document.getElementById("quit-score");
                    var movesInput = document.getElementById("quit-moves");
                    if (scoreInput) scoreInput.value = self.model.totalScore;
                    if (movesInput) movesInput.value = self.model.totalMoves + self.model.roundMoves;
        ...
        handleCardClick(index) {
            ...
            if (result.action === "first-flip") {
                ...
                setTimeout(function () {
                    ...
                    if (result.roundComplete) { self.completeRound(); }
                }, 350);
        ...
        /**
         * Game is over — show end screen, auto-submit to leaderboard
         * after a short delay so the player can see their score.
         */
        endGame() {
            this.stopTimer();
            this.view.showEndScreen(this.model.totalScore, this.model.totalMoves);
